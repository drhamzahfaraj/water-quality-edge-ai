...
We derive analytical expressions for computational cost (FLOPs) and memory footprint of CNN-TCN architectures to quantify efficiency gains over LSTM baselines and validate HW-NAS optimization. For a TCN with $L$ residual blocks, dilation factors $d_\ell = 2^{\ell-1}$, kernel size $k$, and $C$ channels per layer, the total FLOPs per inference are:
\begin{equation}
\text{FLOPs}_{\text{TCN}} = \sum_{\ell=1}^{L} 2 \cdot k \cdot C^2 \cdot T_{\text{out}} \approx 2LkC^2T
\end{equation}
where $T_{\text{out}} \approx T$ for causal convolutions with proper padding. In contrast, LSTM with $H$ hidden units incurs:
\begin{equation}
\text{FLOPs}_{\text{LSTM}} = 4T(P+H)H + 4TH^2
\end{equation}
reflecting four gates (input, forget, output, cell) with input-to-hidden and hidden-to-hidden transformations at each time step \citep{liu2024moderntcn, wang2024tscnd}.

For our configuration ($L=4$, $k=5$, $C=64$, $T=24$, $P=10$), TCN requires $\approx$ 39M FLOPs vs. LSTM with $H=128$ requiring $\approx$ 62M FLOPs, confirming 37\% TCN efficiency. Parameter counts follow $\theta_{\text{TCN}} = 2LkC^2 \approx 163K$ vs. $\theta_{\text{LSTM}} = 4(P+H)H + 4H^2 \approx 270K$, yielding 40\% reduction \citep{yan2024attention}.

Memory footprint under quantization depends on bit-width $b$ and distinguishes weight storage from activation buffering. Weight memory for $b$-bit quantization:
\begin{equation}
M_{\text{weights}}(b) = \frac{b}{8} \cdot \theta_{\text{TCN}} \text{ bytes}
\end{equation}
For $b=8$ (baseline), $M_{\text{weights}} = 163$KB; $b=4$ (aggressive) yields 81.5KB. Activation memory for TCN includes intermediate feature maps:
\begin{equation}
M_{\text{act}} = \sum_{\ell=1}^{L} \frac{b}{8} \cdot C \cdot T_{\ell} \approx \frac{b}{8} \cdot C \cdot T \cdot L
\end{equation}
with $b=8$, $M_{\text{act}} \approx 49$KB; total model size (weights + activations) $\approx$ 212KB for 8-bit, 130KB for 4-bit, vs. LSTM 8-bit $\approx$ 340KB, confirming 38\% TCN memory savings \citep{shabir2024affordable}.

Table \ref{tab:complexity} compares theoretical and measured FLOPs/memory for TCN and LSTM variants under different quantization schemes. Mixed-precision TCN (4--8 bits adaptive) achieves FLOPs parity with fixed 8-bit while reducing average memory 28\% and energy 35\% through variance-driven bit allocation. HW-NAS further reduces FLOPs 18\% by discovering sparse channel configurations ($C \in \{32,48,64\}$ per block) maintaining accuracy within 1\%.

\begin{table}[ht]
\centering
\caption{Computational complexity and memory footprint analysis: TCN vs. LSTM under quantization}
...
Energy estimates apply platform-specific constants ($\alpha_{\text{comp}}=0.12$ nJ/FLOP, $\alpha_{\text{mem}}=2.3$ nJ/byte for Raspberry Pi 4) and account for memory access patterns. TCN's parallel convolutions exhibit better cache locality than LSTM's sequential dependencies, reducing actual energy 8--12\% beyond FLOPs predictions \citep{garavagno2024embedded}. Combined TCN+HW-NAS+adaptive quantization achieves 71\% energy savings vs. LSTM FP32 while improving accuracy 2.2\%, validating synergistic optimization.
...
To assess robustness of design choices, we systematically vary variance thresholds, HW-NAS regularization weights, and quantization configurations, quantifying impact on accuracy and energy. Figure \ref{fig:sensitivity} and Table \ref{tab:sensitivity} summarize results.

\textbf{Variance threshold sensitivity:} Baseline thresholds ($\tau_1=0.05$, $\tau_2=0.15$) for 4/6/8-bit allocation were selected via grid search on validation set. Perturbing by $\pm$20\% ($(0.04, 0.12)$ and $(0.06, 0.18)$) yields RMSE changes of $+0.02$ to $+0.04$ and power variations of $\pm$8\%. Lower thresholds $(0.04, 0.12)$ allocate 8-bit precision more frequently, increasing power 7\% but improving RMSE 3\% (0.60 vs. 0.62). Higher thresholds $(0.06, 0.18)$ favor 4-bit, reducing power 9\% but degrading RMSE 6\% (0.66). The baseline balances accuracy and energy, with performance remaining within 5\% across $\pm$20\% threshold perturbations, demonstrating robustness \citep{gholami2022survey, zhang2024quantedge}.

\textbf{HW-NAS regularization weights:} HW-NAS loss $L_{NAS} = L_{acc} + \lambda_E \cdot E + \lambda_L \cdot \text{Latency}$ with baseline $\lambda_E=0.1$, $\lambda_L=0.05$. Increasing $\lambda_E \to 0.15$ prioritizes energy, yielding 12\% power reduction but 2.1\% accuracy loss (93.2\% vs. 95.0\%). Decreasing $\lambda_E \to 0.05$ improves accuracy marginally (95.3\%) while increasing power 8\%. Latency weight $\lambda_L$ variations ($0.02 \to 0.08$) produce smaller effects ($\pm$3 ms, $\pm$1\% accuracy), confirming energy dominates edge deployment trade-offs \citep{zhou2024survey, garavagno2024embedded}.

\textbf{Fixed vs. adaptive quantization:} Comparing adaptive (4--8 bit variance-driven) against fixed schemes (uniform 4, 6, 8 bit) across test set variance distribution: adaptive matches 8-bit accuracy (95.0\% vs. 95.1\%) while consuming 35\% less power. Fixed 6-bit achieves 92.8\% accuracy with 18\% power savings, underperforming adaptive by 2.2\% accuracy and 17\% energy. Fixed 4-bit reduces power 52\% but suffers 12.7\% accuracy loss (82.3\%), unsuitable for regulatory compliance. Adaptive quantization's variance-aware bit allocation provides Pareto-optimal accuracy-efficiency trade-off \citep{tsanakas2024evaluating, ali2024comprehensive}.

\begin{table}[ht]
\centering
\caption{Sensitivity analysis: impact of variance thresholds and HW-NAS weights on performance}
...
\caption{Sensitivity analysis showing performance stability within $\pm$20\% threshold/weight perturbations. Adaptive quantization (green) dominates fixed schemes across accuracy-power Pareto frontier.}
\label{fig:sensitivity}
\end{figure}

\textbf{Batch size and learning rate robustness:} Training with batch sizes 32, 64, 128 yields RMSE $0.63 \pm 0.02$, confirming stability. Learning rate sweep ($10^{-4}$ to $10^{-2}$) shows optimal convergence at $10^{-3}$ (baseline); $10^{-4}$ requires 50\% more epochs, $10^{-2}$ exhibits instability after epoch 80. Cosine annealing with warm restarts every 30 epochs improves final RMSE 4\% vs. fixed learning rate, escaping local minima \citep{hasan2024optimizing}.
...
Translating our framework to real-world water quality monitoring requires addressing infrastructure, maintenance, communication, and regulatory constraints beyond laboratory validation. We outline key deployment scenarios, integration strategies, and operational requirements for practitioners.

\textbf{Deployment scenarios and environmental adaptation:} Remote river stations face severe power constraints (solar panels 10--20 W with 3--5 days autonomy) and intermittent connectivity (satellite/LoRa 1--4 transmissions/day), favoring aggressive quantization and local inference. Our framework's 20--26 month battery life with 6.5 MB models enables year-round operation in seasonal climates (temperate, tropical monsoon) with biannual maintenance, reducing costs 60\% vs. quarterly servicing for 8--10 month baselines \citep{ken2025integration, simon2025internet}. Urban canal networks with reliable grid power and 4G/5G connectivity support higher sampling rates (15 min vs. hourly) and near-real-time alerts for industrial discharge events, benefiting from 31 Hz capable inference (32 ms latency) and 8-bit precision during high-variance episodes. Reservoir and lake monitoring at 50--200 stations per basin requires federated edge architecture: station-level TCN inference (pollution detection) followed by gateway aggregation (spatial interpolation), exploiting HW-NAS-optimized models' 43M FLOPs efficiency for distributed deployment \citep{liu2025stgcn, babar2024advances}.

\textbf{Energy harvesting and power budgets:} Integration with solar (10--50 W panels, 50--200 Wh battery storage) and micro-hydro generators (5--20 W in flowing rivers) enables perpetual operation when inference energy remains below 10.5 mJ (0.21 W $\times$ 50 ms duty cycle). Our adaptive quantization dynamically allocates precision: 4-bit during stable regimes (nighttime, dry season) conserves energy for communication bursts, 8-bit during pollution events ensures regulatory compliance. Power profiling on Raspberry Pi 4 shows sensor array (pH, DO, turbidity, conductivity) consumes 0.15 W continuous; LoRa transmission (868/915 MHz, 100 mW, 5 s every hour) adds 0.14 Wh/day. Total daily budget: inference 0.21 Wh + sensors 3.6 Wh + communication 0.14 Wh = 3.95 Wh, requiring 40 Wh storage with 20\% safety margin, achievable with 10,000 mAh @ 5V lithium battery and 20 W solar panel (4--6 hours sunlight). Seasonal variation (winter 2--3 h sunlight) may require 30 W panels or reduced sampling (2--4 h intervals) \citep{ken2025integration, simon2025internet}.

\textbf{Communication infrastructure and over-the-air updates:} Remote deployments rely on LoRaWAN (10--15 km range, 0.3--50 kbps), NB-IoT (15--20 km, 20--100 kbps), or satellite (Iridium, Globalstar, 2.4 kbps). Model size directly impacts update feasibility: 6.5 MB compressed TCN requires 15--20 min over LoRa (50 kbps), 50--90 s over NB-IoT (100 kbps), or 40--50 min over satellite (2.4 kbps). Differential updates (transmitting only changed parameters, typically 20--30\% post-distillation fine-tuning) reduce transfer to 1.3--2 MB, enabling quarterly model refreshes over low-bandwidth links. Edge caching of multiple quantization configurations (4-bit, 6-bit, 8-bit variants, total 18 MB) allows runtime switching without re-download, adapting to battery state and detected variance regime \citep{garavagno2024embedded, shen2024edgeqat}.

\textbf{Maintenance intervals and sensor drift:} Battery life analysis assumes 20\% capacity fade over 20--26 months (typical lithium chemistry degradation). Biannual maintenance visits enable battery health checks, sensor calibration (pH drift $\pm$0.1--0.2 units/6 months, DO $\pm$0.3--0.5 mg/L/6 months), and firmware updates. Our framework's extended operational lifetime reduces maintenance frequency 2.5$\times$ vs. fixed 8-bit baselines (8--10 months), translating to 60\% cost savings in remote access (boat/helicopter transport, technician labor). Sensor drift compensation via online calibration (comparing against sporadic grab samples or redundant sensors) requires 1--2\% model capacity reserved for fine-tuning; distillation's accuracy preservation enables this allocation without performance degradation \citep{azmi2024iot, chapman2021water}.

\textbf{Regulatory compliance and uncertainty quantification:} Drinking water quality regulations (WHO, EPA, EU Directive 2000/60/EC) specify maximum contaminant levels (MCLs) for nitrate (50 mg/L), phosphate (context-dependent), turbidity (1--5 NTU), requiring 95\% detection reliability. Our model's 95\% accuracy (±5\% tolerance) meets regulatory thresholds for early warning (trigger confirmatory grab sampling) but not for direct compliance reporting without human validation. Integrating Bayesian neural network extensions or ensemble uncertainty estimates (5-fold cross-validated predictions, standard deviation $\leq$0.08 RMSE) would provide confidence intervals supporting risk-based decisions: high-confidence predictions (95\% CI) enable automated alerts, low-confidence triggers manual sampling, optimizing false alarm rates \citep{chapman2021water, babar2024advances}. Field validation across 3--6 month deployments in representative basins (temperate rivers, tropical lakes, urban estuaries) is needed to calibrate uncertainty estimates and confirm regulatory acceptability.

\textbf{Scalability and cost-effectiveness:} Hardware costs (Raspberry Pi 4: \$55, sensors: \$200--500, solar panel + battery: \$80--150, enclosure + mounting: \$100) total \$435--805 per station. Extended battery life reduces biannual maintenance costs (\$150--300/visit for remote access) by \$300--600 over 2-year lifecycle vs. quarterly servicing, yielding 35--40\% total cost of ownership reduction. For 100-station basin network (\$43.5K--80.5K capital + \$15K--30K annual maintenance with our framework vs. \$50K--90K annual with baselines), savings reach \$35K--60K biennially. Federated learning across stations (future work) could amortize 72 GPU-hour training costs (\$50--150 on cloud GPUs) across entire network, enabling continuous model improvement from collective 20M+ measurements without centralized data aggregation, addressing transboundary data sovereignty concerns \citep{albogami2024adaptive, lang2024comprehensive}.
